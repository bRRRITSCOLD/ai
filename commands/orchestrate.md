...
The security gate is also configurable via `args.securityReview`: `'sensitive'` (default — `security-architect` deep-audits only diffs the staff reviewer flags as security-touching), `'always'` (audit every PR), or `'off'` (staff gate only). Pass it through when the user asks for stricter or looser security review, e.g. `args: { securityReview: 'always' }`.
